// import module for emp management page creation
...
module.exports.demo = (req,res)=>{      
    // to display the employeeManagement page
    Employee.find({},(err,AllEmployee) => {
        res.render('employeeManagement',{AllEmployee:AllEmployee});
    })
};

module.exports.updateEmployeeProfile = (req,res) => { 
    //controller to display a specific employees page to enable editing employees info
...
module.exports.updateEmpInfo_form = (req,res)=>{
     // controller to update employee name and email which are input through form
...
module.exports.updateReviewForm = (req,res)=>{
     //controller to update the review of an employee which is reviewed by someone else
    Employee.findById(req.query.reviewOf,(err,reviewOFF)=>{ 
        // find the employee whose reviewee is to be edited
        for(i of reviewOFF.myReview){ 
       // find from the myReview array of the employee the reviewer whose review is to be updated
...
//check if the admin is deleating itself canbe deleated by other admins
        // console.log('can be deleated');
module.exports.deleteAnEmployee = (req,res) => {
    const toDelete = req.params.id;
    if(toDelete != req.user.id){
